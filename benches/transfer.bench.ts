...
function loadTestWitness(): any {
    ...
    if (!fs.existsSync(inputPath)) {
        throw new Error(
            "Missing input.json - Run circuit setup first to generate valid witness data.\n" +
                "Execute: npm run gen-input:transfer"
        );
    }
...
async function benchmarkWitnessGeneration(
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        memUsages.push((memAfter.heapUsed - memBefore.heapUsed) / 1024 / 1024); // MB
    }
 ...
 */
async function benchmarkProofVerification(
    iterations: number = BENCHMARK_ITERATIONS * 10
): Promise<BenchmarkResult | null> {
...
export async function runAllBenchmarks(): Promise<CircuitBenchmarkResults> {
    console.log("🚀 Starting Transfer Circuit Benchmarks\n");
    ...
    // Save results
    const resultsPath = path.join(BUILD_DIR, "benchmark_results_transfer.json");
