...
describe("Note Circuit Components", function () {
    ...
    describe("NoteCommitment Template", () => {
        ...
        before(async function () {
            ...
            const tempCircuitPath = path.join(
                __dirname,
                "..",
                "build",
        ...
        it("should handle maximum field values", async () => {
            const maxValue =
                "21888242871839275222246405745257275088548364400416034343698204186575808495616"; // Close to field modulus
    ...
    describe("Nullifier Template", () => {
        ...
        it("should compute nullifier correctly", async () => {
            ...
            const expectedNullifier = F.toString(poseidon([commitment, spending_key]));
        ...
        it("should provide unlinkability (same commitment, different keys)", async () => {
            ...
            const diff =
                nullifier1 > nullifier2 ? nullifier1 - nullifier2 : nullifier2 - nullifier1;
    ...
    after(function () {
        // Cleanup temporary test circuits
        cleanupTestCircuits(["test_note_commitment", "test_nullifier"]);
    });
});
