...
async function main() {
    ...
    if (!scenarios.includes(scenario)) {
        console.error(`❌ Invalid scenario: ${scenario}. Must be one of: ${scenarios.join(", ")}`);
        process.exit(1);
    }

    const inputFile = path.join(buildDir, `disclosure_input_${scenario}.json`);
    ...
    console.log(`   Circuit: disclosure (${Object.keys(input).length} signals)`);
